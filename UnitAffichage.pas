...
procedure afficheVille(g: Game; v: Ville); // affiche l'ecran de gestion de la ville
procedure afficheMilitaire(g: Game; c: Civilisation); // affiche l'ecran de gestion militaire
procedure afficheCombat(g: Game; c: Civilisation; niveau: integer; soldatE, canonE: integer); // affiche l'ecran de combat
procedure afficheCivilisation(g: Game; c: Civilisation); // affiche l'ecran de gestion de la civilisation
...
procedure afficheEntete(g: Game; str: String); // affiche l'entete en haut de la fenetre avec str au milieu
begin
  dessinerCadreXY(0, 0, 119, 4, simple, 15, 0);

  deplacerCurseurXY(2, 2);
  write('Civilisation : ', g.Civilisation.nom);
...
procedure afficheSaisie(); // affiche le rectangle en bas a droite et place le curseur au milieu
begin
  dessinerCadreXY(108, 36, 116, 38, simple, 15, 0);
  deplacerCurseurXY(112, 37);
end;

procedure afficheMessage(); // affiche le texte d'erreur eventuel a gauche du rectangle de saisie
begin
...
procedure afficheTitre(str: String); // affiche le titre str au centre de la fenetre
begin
...
procedure afficheDetailVille(v: Ville; decal: integer); // affiche les infos d'une ville
begin
...
begin
  ...
  if (v.caserne > 0) or (v.construction = 2) then
  begin
...
var
  ligne1, ligne2, ligne3, ligne4: string; // chaine contenant la ligne 1, 2, 3 et 4 du detail du combat a afficher
begin
